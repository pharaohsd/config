...
while os.execute("sleep "..INTERVAL) do 
  ...
  data = cpu("cpu0")
  out = "0 widget_tell top cpu0 data user "..data[1].."\n"
  out = out.."0 widget_tell top cpu0 data nice "..data[2].."\n"
  out = out.."0 widget_tell top cpu0 data total "..data[3].."\n"

  data = ethernet("wlan0")
  out = out.."0 widget_tell top down text "..data[1].."\n"
  out = out.."0 widget_tell top up text "..data[2].."\n"  

  data = memory("/proc/meminfo")
  out = out.."0 widget_tell top membar data mem "..data[1].."\n"
  --out = out.."0 widget_tell top membar data swap "..data[2].."\n"

  df_interval = df_interval - 1
  if df_interval < 1 then
    df_data = diskfree("df /dev/sda3 /dev/sda5 /dev/sda2", 80) -- command, range e.g. show (80-100)
    out = out.."0 widget_tell top hddbar data root "..df_data[1].."\n"
    out = out.."0 widget_tell top hddbar data home "..df_data[2].."\n"
    out = out.."0 widget_tell top hddbar data storage "..df_data[3].."\n"
    df_interval = 20 -- only diskfree() each 20'th loop
  end

--  mailbox_interval = mailbox_interval - 1
--  if mailbox_interval < 1 then
--    value = mailbox_count("/home/gig/.mail/default/") -- a mbox
--    out = out.."0 widget_tell top mail text "..value.."\n"
--    mailbox_interval = 10
--  end

  value = os.date("%A %d.%m.%Y %H:%M:%S")
  out = out.."0 widget_tell top clock text "..value.." ".."\n"

  ---------------------------
  -- blinking (alarm) for df
  ---------------------------
  if df_blink[1] then -- reset color
    out = out.."0 widget_tell top pb_df_1 bordercolor root #4444cc".."\n"; df_blink[1] = false
  elseif df_data[1] > 30 then -- blink when value > x
    out = out.."0 widget_tell top pb_df_1 bordercolor root #ff0000".."\n"; df_blink[1] = true
  end
  if df_blink[2] then
    out = out.."0 widget_tell top pb_df_1 bordercolor home #336633".."\n"; df_blink[2] = false
  elseif df_data[2] > 90 then
    out = out.."0 widget_tell top pb_df_1 bordercolor home #ff0000".."\n"; df_blink[2] = true
  end
  if df_blink[3] then
    out = out.."0 widget_tell top pb_df_1 bordercolor storage #663333".."\n"; df_blink[3] = false
  elseif df_data[3] > 90 then
    out = out.."0 widget_tell top pb_df_1 bordercolor storage #ff0000".."\n"; df_blink[3] = true
  end
